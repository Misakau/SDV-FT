...
class SACPolicy(nn.Module):
    ...
    def reset_critic(self, critic1, critic2):
        self.critic1, self.critic1_old = critic1, deepcopy(critic1)
        self.critic1_old.eval()
        self.critic2, self.critic2_old = critic2, deepcopy(critic2)
        self.critic2_old.eval()
    ...
    def get_log_prob(self, obs, actions):
        ...
        if self.action_space:
            action_scale = torch.tensor((self.action_space.high - self.action_space.low) / 2, device=actions.device)
        else:
            action_scale = torch.tensor(1, device=actions.device)
        squashed_action = torch.tanh(actions)
        log_prob = log_prob - torch.log(action_scale * (1 - squashed_action.pow(2)) + self.__eps).sum(-1, keepdim=True)
        return log_prob

    def get_values(self, data):
        obs, actions, next_obs = data["observations"], data["actions"], data["next_observations"]
        
        with torch.no_grad():
            sac_q = torch.min(self.critic1(obs, actions).flatten(), self.critic2(obs, actions).flatten())
            log_probs = self.get_log_prob(obs, actions)
            sac_v = torch.min(
                self.critic1_old(obs, actions), self.critic2_old(obs, actions)
            ) - self._alpha * log_probs
            next_actions, next_log_probs = self(next_obs)
            sac_next_v = torch.min(
                self.critic1_old(next_obs, next_actions), self.critic2_old(next_obs, next_actions)
            ) - self._alpha * next_log_probs
            return sac_q, sac_v, sac_next_v

    def learn(self, data, weights = 1, only_critic=False):
