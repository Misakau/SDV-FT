...
class SegmentTree:
    def __init__(
        self, max_size, observation_dim, action_dim, p_max=1.0
    ):
        ...
        self._types = np.zeros((max_size, 1), dtype="uint8")
    ...
    def update(self, indices, values, is_fake = False):
    ...
    def append(
        ...
        **kwargs
        ...
        self._types[self.index] = sample_type
    ...
    def get(self, data_index):
        ...
        batch["types"] = self._types[data_index]
        return batch
...
class PriorityReplayBuffer:
    def __init__(self, buffer_size, obs_dim, act_dim, p_max=1.0):

        self.obs_dim = obs_dim
        self.act_dim = act_dim

        self.transitions = SegmentTree(
            buffer_size, obs_dim, act_dim, p_max
        )

    def get_offline(self):
        (idxs, ) = np.where(self.transitions._types==1)
        transitions = self.transitions.get(data_index=idxs)
        return transitions
    
    def add(self, obs, next_obs, action, reward, done):
        self.add_sample(obs, action, reward, done, next_obs, 'online')

    def add_sample(
        self, observation, action, reward, terminal, next_observation, sample_type, **kwargs
    ):
        typeid = 1 if sample_type == 'offline' else 0
    ...
    def load_offline(self, offline_buffer):
        ...
        for i in range(offline_size):
            self.add_sample(
                ...
                offline_data['terminals'][i],
                offline_data['next_observations'][i],
                'offline'
            )
        self.transitions.max = offline_size / 1000
    ...
    def update_priorities(self, idxs, priorities):
        if idxs.ndim == 0:
            idxs = np.expand_dims(idxs,0)
            priorities = np.expand_dims(priorities,0)
